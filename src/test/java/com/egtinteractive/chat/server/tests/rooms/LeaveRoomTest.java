...
import static com.egtinteractive.chat.server.tests.resources.TestUtils.CMD_LEAVE;
...
public class LeaveRoomTest {
  ...
  @Test
  public void leaveCurrentRoomTest() throws IOException {
    final String userName = UUID.randomUUID().toString();
    final Client client = new Client(userName, clients, PORT);
    initialize(client);
    createRoom(client, roomName);
    joinRoom(client, roomName, clients);
    client.writeLine(CMD_LEAVE);
    assertEquals(client.readLine(), "You left room:" + roomName);
  }

  @Test
  public void leaveWithoutCurrentRoom() throws IOException {
    final String userName = UUID.randomUUID().toString();
    final Client client = new Client(userName, clients, PORT);
    initialize(client);
    client.writeLine(CMD_LEAVE);
